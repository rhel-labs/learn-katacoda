...
Now, let's run an aggregation SQL workload using sqlcmd as a background task, and monitor SQL plan cache hit ratio using PCP. The query will compute aggregates over 5 million rows from the orders table and use a SQL Server Columnstore index.
...
`(/opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P Redhat1! -i ~/Scripts/CSIndex.sql | grep 'columnstore index' &>/dev/null &) && (pmval -t 1 -T 10  mssql.plan_cache.cache_hit_ratio)`{{execute T2}}
